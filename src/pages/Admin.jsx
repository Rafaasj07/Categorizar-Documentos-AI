...
import AuthStatus from '../components/AuthStatus'; // Adicionado
...
function Admin()  {
    ...
    const handleDeleteSelected = async () => {
        ...
        if (window.confirm(`Tem certeza que deseja apagar ${documentosParaApagar.length} arquivo(s)?`)) {
            setProcessando(true);
            try {
                await apiApagarDocumento(documentosParaApagar); // Chama API para apagar
    ...
    return (
        <div className="flex flex-col min-h-screen bg-black text-white p-4 pb-24 md:pb-4">
            ...
            {/* Código modificado para status authN */}
            <main className="w-full max-w-6xl mx-auto pt-24 md:pt-28">
                <AuthStatus />
                <h1 className="text-3xl md:text-4xl font-bold text-center mb-8">Painel Administrativo</h1>
                <p>Bem-vindo, Administrador! Aqui é possível gerenciar todos os arquivos.</p>
                ...
                <div className="bg-gray-800 p-4 rounded-lg border border-gray-700 mb-8 flex flex-col md:flex-row items-center gap-4">
                    <form onSubmit={handleSearch} className="flex-grow flex items-center gap-4 w-full">
                        <input
                            ...
                            className="w-full flex-grow bg-gray-700 rounded-full py-2 px-4 focus:outline-none focus:ring-2 focus:ring-indigo-500"
