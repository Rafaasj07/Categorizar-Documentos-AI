...
/**
 * Renderiza a página de login, gerencia o estado do formulário
 * e trata a autenticação do usuário.
 */
const Login = () => {
    ...
    // Redireciona o usuário para a home se ele já estiver autenticado.
    useEffect(() => {
        if (isAuthenticated) {
            navigate('/');
        }
    ...
    // Limpa a mensagem de feedback (erro/sucesso) ao focar em um input.
     ...
     * Chama a API de login e trata sucesso ou erro.
     */
    const handleSubmit = async (e) => {
        e.preventDefault();
        limparFeedback();
        try {
            // Tenta realizar o login via contexto de autenticação.
            await login(username, password);
        } catch (err) {
            // Extrai a mensagem de erro da resposta da API ou usa um padrão.
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div>
                        <button 
                            ...
                            disabled={loading}
                        >
                            {/* Mostra ícone de loading ou texto */}
