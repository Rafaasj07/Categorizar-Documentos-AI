...
/**
 * Hook customizado para gerenciar o estado e a lógica
 * de análise e categorização de documentos.
...
export function useDocumentos() {
  ...
  /**
   * Função assíncrona para enviar arquivos para análise da API.
   * Processa múltiplos arquivos sequencialmente.
   */
  const analisarDocumento = async (contextoSelecionado, subContextoSelecionado, promptUsuario, arquivos) => {
    ...
    try {
      ...
      for (let i = 0; i < arquivos.length; i++) {
        ...
        // Adiciona o nome do arquivo local ao objeto de resposta da API.
        const resultadoComNome = { ...respostaApi, nomeArquivo: arquivo.name };

        // Adiciona o resultado do arquivo processado ao estado da lista de documentos.
